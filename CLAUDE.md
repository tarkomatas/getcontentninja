...
- **Two locales, both prefixed:** every page lives under `/hu/…` or `/en/…`. Configured in `astro.config.mjs` (`i18n` with `prefixDefaultLocale: true`). The site root `/` (`src/pages/index.astro`) is a JS language-detecting redirect to `/hu/` or `/en/` (default `/hu/`). **A látható tartalma — logó, márkanév, egy mondat leírás, jogi lábléc — szándékos, és nem törölhető:** a valódi látogató sosem látja (a fejlécben futó `location.replace` előbb elviszi), de minden JS-t nem futtató lekérés (kereső- és AI-crawlerek, hirdetési márkaellenőrzők) pontosan ezt kapja a domain gyökeréről. 2026-09-02-ig egy `noindex, nofollow`-os, két linkből álló üres lap volt itt, ami kívülről parkolt domainnek látszott. A `noindex, nofollow` helyén ezért most `canonical` áll a `/hu/`-ra: duplikált gyökér így sincs, viszont semmi nem tiltja ki az ellenőrzőt, és a crawler tovább tud menni a tartalomra. A sitemapbe emiatt sem kerül be.
...
- **Translated slugs**, mapped in `src/i18n/routes.ts` — the single source of truth for URLs, hreflang alternates (`alternatesFor`), and the language switcher target (`switchTarget`). Change URLs here, not by renaming files ad-hoc. Paths below are written without the trailing slash for readability; the actual values in `routes.ts` all carry it.
  ...
  - `bookDemo` → `/hu/online-bemutato` , `/en/book-demo` (the general **"Bemutatót kérek" / "Book a demo"** landing — hero + lead form covering posting **and** newsletter; every header/footer/homepage demo CTA points here, `lead_forras: 'online-bemutato'`. Az 1. lépésben a négy kiszűrő kérdés után egy `module_interest` kérdés is áll, **max. 2 pipával** — lásd a `ModuleInterestField`-et lentebb)
  ...
  - `ninjaAi` → `/hu/ninja-ai` , **csak magyarul** (`en: null`) — a **beépített AI asszisztens (Ninja AI)** oldala. Nem modul (nincs a `modules.ts`-ben): nem tartalomtípust gyárt, hanem a rendszer *kezelőfelülete* — chatből kezeli a rendszert, válaszol, beállít, elemez. **Indexelhető**, a „Ninja AI" kifejezés definíciós oldala, `FAQPage` JSON-LD-vel. A tartalom az app tényleges működését írja le (`C:\DEV\Content Ninja\docs\copilot-mukodes.md`, `src/lib/chat/agents.ts`, `src/lib/chat/registry/`) — **a számok (12 asszisztens, 160+ művelet) onnan jönnek**, tehát az app bővülésekor itt is frissíteni kell. A fejlécbe/láblécbe szándékosan nincs belinkelve (2026-09-08): előbb a keresőforgalom mérése jön.
  - `woocommerce` → `/hu/woocommerce` , **csak magyarul** (`en: null`) — a **WooCommerce-integráció** SEO/GEO-oldala (2026-09-15), indexelhető, `FAQPage` JSON-LD-vel, `lead_forras: 'woocommerce'`. Azért külön oldal a `webshopIntegration` gyűjtőoldal mellett, mert a „WooCommerce + AI termékleírás/marketing" kifejezésekre egy „webshop mélyintegráció" című lap nem rangsorol, és az AI-keresők a „működik-e WooCommerce-szel?" kérdésre a címében ezt kimondó oldalból válaszolnak. Azért csak magyarul, mert az app a Woo-t magyar opcióként pozicionálja, és a felülete nem angol. Tények: az app `docs/features/woocommerce.md` és `wordpress.md`. WordPressnek szándékosan **nincs** külön oldala — a Blogcikk író oldala viszi.
  - `tiktok` → `/hu/tiktok-posztolas` , **csak magyarul** (`en: null`) — a **TikTok-posztolás** SEO/GEO-oldala (2026-09-25), a `woocommerce` mintájára: indexelhető, `FAQPage` JSON-LD-vel, `lead_forras: 'tiktok'`. **2026-09-26 óta „hamarosan"-oldal** (lásd a posztolási csatornák szakaszát lentebb): jövő időben ír, és nincs rajta súgólink. Nem modul, hanem a posztolás egyik csatornája, ezért a modulrácsból a `posting` marad ki. Tények: az app `docs/features/tiktok.md`-je — **amit nem ígérhet:** zene/TikTok-hang, utólagos szerkesztés/törlés, TikTok Shop, első hozzászólás, Story TikTokra, külön TikTok-szöveg. A TikTok-logó csak eredeti formában, és semmi nem utalhat TikTok-partnerségre. A **YouTube (Shorts)** még átvizsgálás alatt áll: se itt, se máshol ne írd élőnek.
...
- `src/layouts/BaseLayout.astro` — the `<head>` for every page: analytics, inline `tailwind.config`, fonts, canonical + hreflang (computed from `page`+`locale` props), OG tags, and the consent-gated `cookie.js` at end of `<body>`. Props: `locale`, `page`, `title`, `description`, optional `keywords`/`og*`/`canonical`/`noindex`/`bodyClass`/`suppressCanonical` (canonical + hreflang elhagyása — csak a 404 kéri).
...
- `src/pages/404.astro` → `dist/404.html` — **egy fájl mindkét nyelvre**: a GitHub Pages ezt szolgálja ki minden nem létező címre (valódi 404 státusszal), tehát nincs és nem is kell `/hu/404/` + `/en/404/`. Mindkét nyelvi változat benne van a HTML-ben (fejléccel-lábléccel), és egy inline script **kitörli** a feleslegeset a DOM-ból — nem elrejti: a `Header` fix id-ket használ (`mobile-menu-btn` stb.), két példányból a második hamburgere némán halott lenne. A törlés még az elemzés közben fut, a `Header` defer-elt modul-scriptje előtt. JS nélkül a magyar látszik. `suppressCanonical` (lásd `BaseLayout`), mert a 404-nek nincs saját címe.
...
- `public/` — static assets (`assets/`), `CNAME`, `robots.txt`, `sitemap.xml`, the **favicon-készlet** (lásd lentebb), and **legacy redirect stubs** (`bemutato.html`, `adatkezeles.html`, …) that meta-refresh old indexed URLs to the new `/hu/…` paths.
- **Favicon:** a `public/` gyökerében él (`favicon.ico` 16/32/48 + `favicon-16x16.png`, `favicon-32x32.png`, `apple-touch-icon.png`, `icon-192.png`, `icon.png`, `site.webmanifest`), a logó „CN” jeléből generálva: `npm run gen:favicon` (`scripts/gen-favicon.mjs`, sharp). A **vektoros** változat (`favicon.svg` és a vele azonos `icon.svg` — az automata ellenőrzők ezt a nevet is próbálják) az app repó `public/brand/logo-mark.svg`-jéből jött, a nyitó tagja normalizálva (nincs fix `width`/`height`, nincs `preserveAspectRatio="none"`). A `<link rel="icon">` sorok **két helyen** kellenek: a `BaseLayout.astro`-ban és külön a `src/pages/index.astro`-ban (a gyökér-átirányító nem használ layoutot, viszont épp azt az URL-t hívja meg először minden márkaellenőrző). 2026-09-01-ig egyik sem létezett, és az OpenAI emiatt utasította el a hirdetési márkaellenőrzést — nem elég a fájl, a HTML-hivatkozás is kell.
...
- **Támogatott platformok (Unas, Shoprenter, WooCommerce, WordPress) – EGY forrás:** `src/data/platforms.ts`, képességjelzőkkel (`products`, `writeBack`, `audit`, `blog`, `chatgptFeed`); a logósorokat a `src/components/PlatformLogos.astro` rakja ki (`cap` szerint szűrve — a blog-kontextus pl. Unas + Shoprenter + WordPress, a Shopgrade-é Unas + Shoprenter + WooCommerce), és ebből épül a fejléc integrációs sávja is. Korábban a logópár 16 fájlban volt kézzel beégetve. **Új platformnál:** bővítsd a `platforms.ts`-t, majd keress rá a folyó szövegben a platformnevekre (címek, GYIK, `has_webshop` opciók) — azok kézzel állnak. A WooCommerce és a WordPress az appban **egy kapcsolat** (a saját „Content Ninja" WordPress-bővítménnyel), ezért a blog-publikálásnál a WordPress áll, nem a WooCommerce. A `has_webshop` új értékei (`WooCommerce`, a Blogcikk író űrlapján `WordPress (webshop nélkül)`) — a többihez hasonlóan — **nem fordulnak és nem nevezhetők át** (az app egy oszlopban szűri őket); a bevezetésükkor a `QUALIFY_KEY` `cn_qualify_v3`-ra lépett.
- **Posztolási csatornák (Facebook, Instagram, TikTok, YouTube Shorts) – állapot EGY helyen:** `src/data/socialPlatforms.ts` (`live` | `soon`). **Ma csak a Facebook és az Instagram él**; a TikTok elkészült, de még nem elérhető az előfizetőknek, a YouTube (Shorts) a platform átvizsgálása alatt áll. `soon` csatornát a honlap **sehol nem írhat élőnek** — csak „hamarosan"/„coming soon"-ként. A logósorokban a `soon` csatorna mellé a `SoonBadge.astro` kerül (`{isSoon('tiktok') && <SoonBadge … />}`); a logót nem szürkítjük el (márkaszabály). A jogi oldalak (ÁSZF, adatkezelés, adattörlés) TikTok/YouTube-szakaszai szándékosan maradnak: az adatkezelést írják le a bekötés esetére. **Élesedéskor:** `socialPlatforms.ts`-ben `live`, majd keress rá a csatorna nevére + „hamarosan"/„coming soon"-ra (meta description, GYIK, JSON-LD, `modules.ts`), és a `/hu/tiktok-posztolas/` oldalt írd vissza jelen időbe.
...
- a gyűjtő **mezőnként EGY értéket** olvas (`el.value`), ezért többválasztós kérdéshez jelölőnégyzetek
  ...
  a kipipáltakat. Ha a jelölőnégyzetek maguk kapnának `name`-et, a gyűjtő a kipipálatlanokat is
  kitöltöttnek látná, és csak az utolsó érték maradna meg.
- ez a trükk a **`src/components/ModuleInterestField.astro`** komponensben él (`label`, `hint`, `max`,
  `options` propok) — négy oldal használja: a `bookDemo` (`online-bemutato`/`book-demo`, **max. 2 pipa**)
  és az `allInOne` (`teljes-rendszer`/`all-in-one`, max. 3) oldalpár. A `max` fölött a maradék
  jelölőnégyzet letiltódik, tehát a válasz tényleg prioritást mond, nem kívánságlistát. **Nincs rajta
  `data-reject`**, a minősítést nem befolyásolja. ⚠️ **A `value`-k mindig magyarok és változatlanok**
  (a látható címke fordul csak): az app egy `module_interest` oszlopban szűri őket, egy átnevezés
  kettévágná a régi és az új leadeket. Ahol a form `max-h-[500px] overflow-hidden`-t kapott, ott a
  mezővel együtt a plafont is le kell venni — különben levágja a „Tovább" gombot.
