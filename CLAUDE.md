...
- **Favicon:** a `public/` gyökerében él (`favicon.ico` 16/32/48 + `favicon-16x16.png`, `favicon-32x32.png`, `apple-touch-icon.png`, `icon-192.png`, `icon.png`, `site.webmanifest`), a logó „CN” jeléből generálva: `npm run gen:favicon` (`scripts/gen-favicon.mjs`, sharp). A `<link rel="icon">` sorok **két helyen** kellenek: a `BaseLayout.astro`-ban és külön a `src/pages/index.astro`-ban (a gyökér-átirányító nem használ layoutot, viszont épp azt az URL-t hívja meg először minden márkaellenőrző). 2026-09-01-ig egyik sem létezett, és az OpenAI emiatt utasította el a hirdetési márkaellenőrzést — nem elég a fájl, a HTML-hivatkozás is kell.
