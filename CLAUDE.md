...
- **Translated slugs**, mapped in `src/i18n/routes.ts` — the single source of truth for URLs, hreflang alternates (`alternatesFor`), and the language switcher target (`switchTarget`). Change URLs here, not by renaming files ad-hoc. Paths below are written without the trailing slash for readability; the actual values in `routes.ts` all carry it.
  ...
  - `tiktok` → `/hu/tiktok-posztolas` , **csak magyarul** (`en: null`) — a **TikTok-posztolás** SEO/GEO-oldala (2026-09-25, a TikTok jóváhagyásának napján), a `woocommerce` mintájára: indexelhető, `FAQPage` JSON-LD-vel, `lead_forras: 'tiktok'`. Nem modul, hanem a posztolás egyik csatornája, ezért a modulrácsból a `posting` marad ki. Tények: az app `docs/features/tiktok.md`-je — **amit nem ígérhet:** zene/TikTok-hang, utólagos szerkesztés/törlés, TikTok Shop, első hozzászólás, Story TikTokra, külön TikTok-szöveg. A TikTok-logó csak eredeti formában, és semmi nem utalhat TikTok-partnerségre. A **YouTube (Shorts)** még átvizsgálás alatt áll: se itt, se máshol ne írd élőnek.
